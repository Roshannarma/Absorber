...
@SpirePatch(    // "Use the @SpirePatch annotation on the patch class."
        clz = SingleCardViewPopup.class, // This is the class where the method we will be patching is. In our case - Abstract Dungeon
        method = "renderFrame"// This is the name of the method we will be patching.
        /*
...
public class LargeMonsterFramePatch {// Don't worry about the "never used" warning - *You* usually don't use/call them anywhere. Mod The Spire does.
    ...
    private static final Logger logger = LogManager.getLogger(LargeMonsterFramePatch.class.getName());
    private static final Texture CARD_FRAME_MONSTER_ATTACK_L = new Texture("AbsorberResources/images/Rarity/Attack_p.png");
    private static final TextureAtlas.AtlasRegion CARD_FRAME_MONSTER_ATTACK_L_ATLAS = new TextureAtlas.AtlasRegion(CARD_FRAME_MONSTER_ATTACK_L, 0, 0, CARD_FRAME_MONSTER_ATTACK_L.getWidth(), CARD_FRAME_MONSTER_ATTACK_L.getHeight());
    private static final Texture CARD_FRAME_MONSTER_SKILL_L = new Texture("AbsorberResources/images/Rarity/Skill_p.png");
    private static final TextureAtlas.AtlasRegion CARD_FRAME_MONSTER_SKILL_L_ATLAS = new TextureAtlas.AtlasRegion(CARD_FRAME_MONSTER_SKILL_L, 0, 0, CARD_FRAME_MONSTER_SKILL_L.getWidth(), CARD_FRAME_MONSTER_SKILL_L.getHeight());
    private static final Texture CARD_FRAME_MONSTER_POWER_L = new Texture("AbsorberResources/images/Rarity/Power_p.png");
    private static final TextureAtlas.AtlasRegion CARD_FRAME_MONSTER_POWER_L_ATLAS = new TextureAtlas.AtlasRegion(CARD_FRAME_MONSTER_POWER_L, 0, 0, CARD_FRAME_MONSTER_POWER_L.getWidth(), CARD_FRAME_MONSTER_POWER_L.getHeight());



    // This is our logger! It prints stuff out in the console.
    // It's like a very fancy System.out.println();
//
//    @SpirePrefixPatch( // This annotation of our patch method specifies the type of patch we will be using. In our case - a Spire Insert Patch
//            // Spire insert patches require a locator - this isn't something you import - this is something we write.
...
//
//            // The method we're patching, returnRandomRelicKey(), has a local variable that we'd like to access and manipulate -
...
//    )
    ...
    @SpireInsertPatch(
            rloc = 3,
            localvars = {"tWidth","tOffset","tmpImg"})
    public static SpireReturn FunctiontoAppend(SingleCardViewPopup __instance, SpriteBatch sb, @ByRef AbstractCard ___card[], @ByRef float tWidth[], @ByRef float tOffset[], @ByRef TextureAtlas.AtlasRegion tmpImg[]
...
//        logger.info(___card[0].rarity);
        if(___card[0].rarity == MonsterRarityEnum.MONSTER){
//            logger.info(___card[0].type);
...
//            case BASIC:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_COMMON, drawX, drawY);
//                return;
//            case CURSE:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_COMMON, drawX, drawY);
//                return;
//            case SPECIAL:
//                Texture MONSTER_BANNER = new Texture("AbsorberResources/images/Rarity/Banner.png");
//                TextureAtlas.AtlasRegion MONSTER_BANNER_ATLAS = new TextureAtlas.AtlasRegion(MONSTER_BANNER, 0, 0, MONSTER_BANNER.getWidth(), MONSTER_BANNER.getHeight());
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), MONSTER_BANNER_ATLAS, drawX, drawY);
//            case COMMON:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_COMMON, drawX, drawY);
//                return;
//            case UNCOMMON:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_UNCOMMON, drawX, drawY);
//                return;
//            case RARE:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_RARE, drawX, drawY);
//            default:
//                AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), ImageMaster.CARD_BANNER_COMMON, drawX, drawY);
//        }
