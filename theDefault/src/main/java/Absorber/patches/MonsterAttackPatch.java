...
@SpirePatch(    // "Use the @SpirePatch annotation on the patch class."
        ...
        method = "renderAttackPortrait"// This is the name of the method we will be patching.
        /*
...
public class MonsterAttackPatch {// Don't worry about the "never used" warning - *You* usually don't use/call them anywhere. Mod The Spire does.
    ...
    private static final Logger logger = LogManager.getLogger(BannerImagePatch.class.getName());
    private static final Texture MONSTER_BANNER = new Texture("AbsorberResources/images/Rarity/Attack1.png");
...
//    )
    ...
    @SpirePrefixPatch
    public static SpireReturn FunctiontoAppend(AbstractCard __instance, SpriteBatch sb, float x, float y
...
//        logger.info(__instance.rarity);
        if(__instance.rarity == MonsterRarityEnum.MONSTER){
//            logger.info(__instance.rarity);
//            this.renderHelper(sb, this.renderColor, ImageMaster.CARD_FRAME_ATTACK_RARE, x, y);
            AbsorberRenderHelper.helper(__instance, sb, Color.WHITE.cpy(), MONSTER_BANNER_ATLAS, x, y);
